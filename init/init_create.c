...
/*****************************************************************\
  Create a database - switches are:
    -v volume set name           (1 to VAR_LEN chars)         Reqd
    -b block size in KiB         (4 to 256)                   Reqd
    -s database size in blocks   (100 to MAX_DATABASE_BLKS)   Reqd
...
{ int namlen;                                   // length of volume name
  ...
  int i;                                        // for loops
  union temp_tag
  { int buff[131072];                           // 512 KiB buffer
    char cuff[sizeof(label_block) + 1];         // remap front bit for label block + 1 for first map block byte
  ...
  { map = (blocks + 7) / 8 + 1 + sizeof(label_block); // see what we need
  }
  if (map & 1023)				// if not even KiB
  { map = (map / 1024 + 1) * 1024;		// round up
  }
  if (map < bsize) map = bsize;                 // at least bsize
  if (map < (blocks + 7) / 8 + 1 + sizeof(label_block)) // if less than reqd
  ...
  printf("with %d x %d KiB blocks ", blocks, bsize / 1024);
  ...
  if (map > (512 * 1024))
  { ret = write(fid, x.buff, (512 * 1024));     // write out the first 512 KiB incl. header + first map block byte
  ...
  memcpy(&x.cuff[sizeof(DB_Block)], &i, sizeof(short)); // save in index
