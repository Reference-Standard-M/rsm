...
{ int dbfd;                                     // database file descriptor
  ...
  { fprintf(stderr, "Database is version %d, image requires version %d - start failed!!\n", labelblock->db_ver, DB_VER);
  ...
  printf("Creating share for %d jobs with %d MiB routine space,\n", jobs, rmb);
  printf("%d MiB (%d) global buffers, %d KiB label/map space\n", gmb, n_gbd, hbuf[2] / 1024);
  printf("and %d KiB for the lock table.\n", locksize / 1024);
  if (addmb > 0) printf("With %d MiB of additional buffer.\n", addmb);
  ...
  volset_size = sizeof(vol_def)			// size of VOL_DEF (one for now)
	      ...
	      + (n_gbd * sizeof(struct GBD))	// the gbd
              + (gmb * MBYTE)		  	// MiB of global buffers
              + hbuf[3]			       	// size of block (zero block)
              + (rmb * MBYTE);		 	// MiB of routine buffers
  ...
  shar_mem_id = shmget(shar_mem_key, share_size, (SHM_R | SHM_W | (SHM_R >> 3) | (SHM_W >> 3) | IPC_CREAT)); // create share memory
  ...
  { fprintf(stderr, "Unable to create shared memory section - %s\n", strerror(errno)); // give an error
    ...
    if (errno == EINVAL)
#ifdef __APPLE__
      fprintf(stderr, "\tIncrease kern.sysv.shmmax and kern.sysv.shmall\n");
#else
  ...
  systab->vol[0]->zero_block = (void *) &(((u_char *) systab->vol[0]->global_buf)[gmb * MBYTE]); // pointer to zero blk
