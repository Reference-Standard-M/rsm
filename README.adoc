...
*Version 1.76.2 - 2022 Feb 23*

=== Implementation of ANSI Standard M X11.1-1995 (ISO/IEC 11756:1999)
...
[source,m]
----
RSM [MGR]> merge ^$routine("%RECOMP")=^$routine("%RECOMP")
RSM [MGR]> do ^%RECOMP
----
...
To create and run it as a daemon:
...
To run RSM in direct mode while in daemon mode:
...
To run RSM commands while in daemon mode:
...
To run the Bash shell while in daemon mode:
...
To start the container while in daemon mode:
...
To stop the container while in daemon mode:
