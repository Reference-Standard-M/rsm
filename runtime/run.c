...
{ int opc;					// current opcode
  ...
  { if (ssp >= MAX_SSTK)			// check ssp
    ...
    { s = attention();				// do it
      ...
      { savasp = partab.jobtab->dostk[partab.jobtab->cur_do].asp;
        ...
        { partab.jobtab->error_frame = partab.jobtab->cur_do; // set $ST(-1)
          bcopy(&partab.jobtab->dostk[partab.jobtab->cur_do], &partab.jobtab->dostk[STM1_FRAME], sizeof(do_frame)); // save for ron
	}
	if (s == -(ERRZ51 + ERRMLAST))		// if it's a control c
	...
	{ t = 0;				// ignore undefined
        ...
        ssp = ssp + cptr->len + sizeof(u_short) + 1; // point past it
	p = &strstk[ssp];			// some space
	comp_ptr = p;				// call it a compile ptr

	if (cptr->len && !flag)			// if there is something there and no prev err at this lev
	...
	*comp_ptr++ = ENDLIN;			// JIC
	...
	ssp = ssp + sizeof(u_short) + 2;	// point past it
	...
	ssp = ssp + cptr->len + sizeof(u_short) + 1; // point past it (sort of)
	addstk[asp++] = (u_char *) cptr;	// stack it
	break;
	...
	cptr->len = s;				// save the length
	ssp = ssp + s + sizeof(u_short) + 2;	// point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	break;
	...
	ssp = ssp + s + sizeof(u_short) + 1;	// move ssp along
	...
	if (opc == CMSETP)			// need a delimiter?
	  t = DSetpiece(p, cptr, var, ptr1, i, j); // do a SET $P
	else					// must be set $E()
	  t = DSetextract(p, cptr, var,	i, j);	// do a SET $E
	if (t < 0) ERROR(t)			// complain on error
	break;
	...
	{ tmp = (cstring *) addstk[--asp];	// get an argument
	  ...
	  else ERROR(-(ERRZ13 + ERRMLAST))	// else error
	}
	...
	{ bcopy(&partab.src_var, &partab.jobtab->last_ref, sizeof(var_u) + 5 + partab.src_var.slen); // update naked
	  t = DB_Get(&partab.src_var, cptr->buf); // do it - global
        ...
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// save the length
	ssp = ssp + t + sizeof(u_short) + 1;	// point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	break;
...
// *************** Start of functions ***************
	...
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dextract(cptr->buf, (cstring *) addstk[--asp], 1, 1); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	i = cstringtoi((cstring *) addstk[--asp]); // get second arg
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dextract(cptr->buf, (cstring *) addstk[--asp], i, i); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	i = cstringtoi((cstring *) addstk[--asp]); // get second arg
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dextract(cptr->buf, (cstring *) addstk[--asp], i, j); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	t = Dfind2(cptr->buf, (cstring *) addstk[--asp], ptr1); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	ptr1 = (cstring *) addstk[--asp];	// get arg 2
	t = Dfind3(cptr->buf, (cstring *) addstk[--asp], ptr1, i); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dfnumber2(cptr->buf, ptr1, ptr2);	// doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	ssp = ssp + sizeof(u_short) + ptr1->len + 1; // point past it
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dfnumber3(cptr->buf, ptr1, ptr2, i); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dget2(cptr->buf, var, ptr1);	// doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	i = cstringtoi((cstring *) addstk[--asp]); // get second arg
	t = Djustify2(cptr->buf, (cstring *) addstk[--asp], i); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	ssp = ssp + sizeof(u_short) + ptr1->len + 1; // point past it
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Djustify3(cptr->buf, ptr1, i, j); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	ptr1 = (cstring *) addstk[--asp];	// get arg 2
	t = Dpiece2(cptr->buf, (cstring *) addstk[--asp], ptr1); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	i = cstringtoi((cstring *) addstk[--asp]); // get third arg
	ptr1 = (cstring *) addstk[--asp];	// get arg 2
	t = Dpiece3(cptr->buf, (cstring *) addstk[--asp], ptr1, i); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	i = cstringtoi((cstring *) addstk[--asp]); // get third arg
	ptr1 = (cstring *) addstk[--asp];	// get arg 2
	t = Dpiece4(cptr->buf, (cstring *) addstk[--asp], ptr1, i, j); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	{ if ((var->uci == UCI_IS_LOCALVAR) || (var->uci == 0)) // if it's local or no UCI there
	  ...
	  list = &systab->vol[0]->vollab->uci[var->uci - 1].name; // UCI table
	  for (args = 0; args < VAR_LEN; args++) // use a random int
	  { if (list->var_cu[args] == 0) break; // quit on null
	    cptr->buf[i++] = list->var_cu[args]; // copy a character
	  }
	...
	break;
      ...
      case FUNRE:				// $RE[VERSE]
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dreverse(cptr->buf, (cstring *) addstk[--asp]); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	ptr1 = (cstring *) addstk[--asp];	// get arg 2
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dtranslate2(cptr->buf, (cstring *) addstk[--asp], ptr1); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	ptr1 = (cstring *) addstk[--asp];	// get arg 2
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dtranslate3(cptr->buf, (cstring *) addstk[--asp], ptr1, ptr2); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Dview(cptr->buf, cstringtoi((cstring *) addstk[--asp]), i, j, ptr1); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// the count
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
...
// *************** End of functions ***************
	...
	// Note: The above two mvars have been pre-expanded to maximum size (ie. sizeof(mvar)).

	if (var->name.var_cu[0] == '$') 	// dest is SSVN
	{ if (toupper(var->name.var_cu[1]) != 'R')
	    ERROR(-ERRM29)			// must be ^$R()
	  t = Compile_Routine(var,		// compile this routine
			      var2,		// from here
	...
	if ((t < 0) && (t != -ERRM6) && (t != -ERRM7))
	  ERROR(t)				// error other than UNDEF
	if (t >= 0)				// was defined
	{ cptr->len = t;			// save the length
	...
	{ if (var2->uci == UCI_IS_LOCALVAR)
	  ...
	  if ((i + var2->slen - j) > 255)
	    ERROR(-(ERRZ2 + ERRMLAST))		// complain if too big
	...
	if (!*p) p++;				// skip possible eol
	if (*p++ == LINENUM)
	  curframe->line_num = *(u_short *) p;  // store the line number
	...
	if (*p++ == LINENUM)
        { assert(sizeof(s) == sizeof(short));
	  bcopy(p, &s, sizeof(short));
          p += sizeof(short);
          curframe->line_num = (u_short) s;
	...
	*comp_ptr++ = ENDLIN;			// JIC
	...
	partab.jobtab->dostk[partab.jobtab->cur_do].symbol = NULL; // clear pointer to prevent double free bugs
	...
	if (opc == CMQUITA)			// if there was an arg
	{ ptr1 = (cstring *) &strstk[ssp];	// where we will put it
	  bcopy(cptr, ptr1, cptr->len + sizeof(u_short) + 1); // copy it
	  ssp = ssp + ptr1->len + sizeof(u_short) + 1; // protect it
	...
	{ var = (mvar *) addstk[--asp];		// get next from list
	  ...
	  if (var->slen)			// any subscripts
	    ERROR(-(ERRZ13 + ERRMLAST))		// not permitted
	...
	break;
      ...
      case LINENUM:				// set current line number
        ...
        bcopy(rsmpc, &s, sizeof(short));
        rsmpc += sizeof(short);
        partab.jobtab->dostk[partab.jobtab->cur_do].line_num = (u_short) s;
        //*((short *) rsmpc)++;		// store the line number
        bcopy(rsmpc, &s, sizeof(short));
        partab.jobtab->dostk[partab.jobtab->cur_do].endlin = rsmpc + (u_short) s;
        rsmpc += sizeof(short);
        //*((short *) rsmpc)++;			// store the line number
	...
	{ var = (mvar *) &strstk[ssp];          // somewhere to put it
	  ...
	  if (var->uci != UCI_IS_LOCALVAR)	// not a local?
	    ERROR(-(ERRZ13 + ERRMLAST))		// must be local
	  ssp = ssp + var->slen + sizeof(var_u) + (sizeof(u_char) * 4);
	  s = -1;				// flag var type
	}
	forx = (for_stack *) &strstk[ssp];	// where to put for stuff
	ssp = ssp + sizeof(for_stack);		// protect it
...
// ************** Indirection stuff **************
      case INDREST:				// restore isp and rsmpc
        ...
        bcopy(rsmpc, &rsmpc, sizeof(u_char *));
...
//    case XCSIG:				// Xcall $&%SIGNAL() -	done elsewhere
	...
	ptr1 = (cstring *) addstk[--asp];	// get arg 1
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Xcall_v((char *) cptr->buf, ptr1, ptr2); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// save the length
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	ptr1 = (cstring *) addstk[--asp];	// get arg 1
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Xcall_x((char *) cptr->buf, ptr1, ptr2); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// save the length
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
	...
	ptr1 = (cstring *) addstk[--asp];	// get arg 1
	cptr = (cstring *) &strstk[ssp];	// where we will put it
	t = Xcall_getenv((char *) cptr->buf, ptr1, ptr2); // doit
        if (t < 0) ERROR(t)			// complain on error
	cptr->len = t;				// save the length
	ssp = ssp + sizeof(u_short) + cptr->len + 1; // point past it
	addstk[asp++] = (u_char *) cptr;	// stack it
