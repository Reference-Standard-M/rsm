...
This version reverted the previous version's change to the bytecode format. If
...
* Improve error handling of tag offsets that are too large
* Fix bug in debugger that prevented proper reset in certain scenarios
...
* Change database permissions on creation to 640 to match the journal file
* Improve fix to double free bug preventing a possible leak during error frames
* Fix bug that prevented display of some error messages when starting a job
...
* Fix bug that mangled arguments to the `job` command when run via `rsm -x`
...
* Fix bug that opened the database file a second time during a `job` command
* Fix bug in the `$&paschk` XCall that closed the password file prematurely
...
* Fix bug in vendor global lister utility routine [`%G`]
...
* Fix bug on Cygwin that prevented a privileged job from shutting RSM down
