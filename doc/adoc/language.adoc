...
RSM currently supports only one volume (database and after-image journal file)
per volume set, though that will change in the future. Each volume is created
with an initial manager UCI [User Class Identifier], which is named `MGR` by
default, but which can be named at volume creation with the `-e` option. Each
volume can also contain up to a total of 63 UCIs, including the default manager
UCI.
...
either the user who started the RSM environment, a member of the group that
started the RSM environment, or the '`root`' user.
...
NOTE: Structured system variables are case-insensitive, and have a long and
short form.
...
|`_global_`            |Top root block number of _global_    |No
...
|`"WORD_SIZE"`                    |Machine word size          |No
